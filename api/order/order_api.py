...
def pay_data():
    ...
    try:
        ...
        cursor.execute(query_history,(find_id,))
        ...
        if find_history==None:
            ...
            cursor.execute(query,(order_id,member_id,member_name,member_email,member_phone,order_price))
            ...
            if record["status"]==0:
                final_status=0
                change_status=("update orders set pay_status = %s where order_id =%s;")
            ...
            cursor.execute(delete_history,(find_id,))
            connection_object.commit()
            member_id=find_id["id"]
            member_name=data["contact"]["name"]
            member_phone=data["contact"]["phone"]
